    ...
    <div class="container">
      <h3>Size </h3>
        <div class="form-group">
              <a href="{{ url('admin/sizes/create') }}" class="btn btn-primary bg-color  btn-section">Add New Size</a>
            </div>
      ...
      <tbody>
        ...
        <tr class="gradeX" id="row_{{$size->id}}">
           <td>{{$size->size_name}}</td>
            <td>{{$size->brand_name}}</td>
          <td>{{$size->category_name}}</td>
           <td>{{$size->sub_category_name}}</td> 
          <td class="actions">
            &nbsp;&nbsp;&nbsp;
            <span>
              <a href="{{URL::to('admin/sizes/edit')}}/{{$size->id}}"> <i class="fa fa-pencil bg-orange" aria-hidden="true"></i> Edit
              </a>
            </span>
            <a  style="cursor:pointer;" onclick="deleteStatus({{$size->id}});"><i class="fa fa-trash bg-orange" aria-hidden="true"></i>
              Delete
            </a>
            &nbsp;&nbsp;&nbsp;
          </td>
