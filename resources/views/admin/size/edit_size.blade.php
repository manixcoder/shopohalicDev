...
<div class="right-marchent-wapper">
          <div class="product-detailform">
            ...
            <form method="POST" action="{{ url('/admin/sizes/'.$sizeData->id.'/update') }}" enctype="multipart/form-data">
            {{ csrf_field() }}
            <div class="row">
                <div class="col-md-12 col-sm-12">
                  <div class="form-group">
                    <label>Size Name</label>
                    <input class="form-control" type="text" id="size_name" name="size_name" value="{{$sizeData->size_name}}" placeholder="Size Name">
                  </div>
                </div>
              ...
              <div class="col-md-12 col-sm-12">
                  <div class="form-group">
                   ...
                   <select  class="form-control" id="category_id" name="category_id" onchange="getSubCategory(this.value),getBrand('category',this.value);">
                    ...
                    <option value="{{$category->id}}" @if($sizeData->category_id==$category->id) selected @endif>{{$category->category_name}}</option>
                    @endforeach
                   </select>
                  </div>
                </div>
              ...
              <div class="col-md-12 col-sm-12">
                  <div class="form-group">
                    ...
                    <option value="{{$subcategory->id}}" @if($sizeData->sub_category_id==$subcategory->id) selected @endif>{{$subcategory->category_name}}</option>
                    @endforeach                
                   </select>
                  </div>
                </div>
                <div class="col-md-12 col-sm-12">
                  <div class="form-group">
                    ...
                    <option value="{{$brand->id}}" @if($sizeData->brand_id==$brand->id) selected @endif>{{$brand->brand_name}}</option>
                    @endforeach                     
                   </select>
                  </div>
                </div>
